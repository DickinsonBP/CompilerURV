...
%token <var.st.data_value.boolean> BOOLEAN
...
%token NOT AND OR
%token REPEAT DO DONE SPACE IF THEN ELSE FI WHILE UNTIL FOR IN

%type <var.st> programa
%type <var> exp_aritm
%type <var> if_else else repeat_loop while_loop do_loop for_loop
%type <var> sentencias sentencia or and boolval
%type <var.st> valor
%type <var.st> sumaresta mult pow
%type <var.st.data_value.entero> m
%type <var.st.data_value.ident.lexema> ops
...
exp_aritm : ID_ARITM ASSIGN sumaresta ENDLINE{
  ...
  if(type == STRING_TYPE){
    ...
    $3.data_value.identifier_type = BOOL_TYPE;
  }
  if(type == BOOL_TYPE){
    emet($1.data_value.ident.lexema, 0, &$3, NULL, NULL);
    $3.data_value.identifier_type = BOOL_TYPE;
  }
...
boolval : BOOLEAN                { $$.st.type_value = BOOL_TYPE; $$.st.data_value.boolean = $1;}
        | OP or CP               { $$ = $2; }
        | sumaresta ops sumaresta { $$.true_list = newList(instructionLine); $$.false_list = newList(instructionLine + 1); emet_if($2, &$1, &$3,0); emet_ir(0); }


or : or OR m and   { $$.true_list = join($1.true_list, $4.true_list); $$.false_list = $4.false_list; completar($1.false_list, $3); }
    | and

and : and AND m boolval    { $$.true_list = $4.true_list; $$.false_list = join($1.false_list, $4.false_list); completar($1.true_list, $3); }
    | NOT boolval          { $$.true_list = $2.false_list; $$.false_list = $2.true_list; }
    | boolval

if_else : IF boolval THEN ENDLINE m sentencias else FI ENDLINE {
      completar($2.true_list, $5);
      if($7.st.data_value.entero == -1){
        $$.next_list = join($2.false_list, $6.next_list);
      }else{
        $$.next_list = join($6.next_list, $7.next_list);
        completar($2.false_list, $7.st.data_value.entero);
      }
      
    }

else : ELSE ENDLINE m sentencias { $$.next_list = $4.next_list; }
      |              { $$.true_list = newEmptyList(); }

while_loop : WHILE m boolval DO ENDLINE m sentencias DONE ENDLINE{
                                    completar($3.true_list, $6);
                                    completar($7.next_list, $2);
                                    emet_ir($2);
                                    $$.next_list = $3.false_list;
                                  }

repeat_loop : REPEAT m sumaresta DO ENDLINE m sentencias DONE ENDLINE { 
                                    completar($7.next_list, $2);
                                    emet_if("LT",&$3,0,instructionLine + 2);
                                    emet_ir(0);
                                  }

do_loop : DO ENDLINE m sentencias UNTIL m boolval ENDLINE {
                                       completar($4.next_list, $6);
                                       completar($7.true_list, $3);
                                       emet_ir(0);
                                       $$.next_list = $7.false_list;
                                     }

for_loop : FOR ID_ARITM IN sumaresta DO ENDLINE sentencias DONE ENDLINE {
